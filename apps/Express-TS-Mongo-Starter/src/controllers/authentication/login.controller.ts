...
import { DEFAULT_TOKEN_EXPIRATION_LONG, DEFAULT_TOKEN_EXPIRATION_SHORT } from '../../utils/common.util.js';
...
const loginController = async (req: Request, res: Response) => {
  try {
    ...
    const hashedPassword = await PasswordUtil.comparePassword(existingUser.password, password);
    ...
    const tokenExpiresIn = keepLogin ? DEFAULT_TOKEN_EXPIRATION_LONG : DEFAULT_TOKEN_EXPIRATION_SHORT;
    ...
    if (!token) {
      return res.status(500).send({ message: "Can't create login token!", success: false });
    }
